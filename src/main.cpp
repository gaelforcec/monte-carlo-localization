...
/////
std::shared_ptr<pros::Imu> imu1 = std::make_shared<pros::Imu>(1);  // Port 1
std::shared_ptr<pros::Imu> imu2 = std::make_shared<pros::Imu>(2);  // Port 2

// Tracking wheels
std::shared_ptr<pros::Rotation> forwardTracker = std::make_shared<pros::Rotation>(3);  // Port 3
std::shared_ptr<pros::Rotation> horizontalTracker = std::make_shared<pros::Rotation>(4);  // Port 4

// Distance sensor
std::shared_ptr<pros::Distance> distanceSensor = std::make_shared<pros::Distance>(5);  // Port 5

// Then create the MCLLocalizer
auto mcl = std::make_shared<MCLLocalizer>(
    imu1,           // First IMU
    imu2,           // Second IMU
...
void initialize() {
  ...
  // Make sure IMUs are calibrated before using
  imu1->reset();
  imu2->reset();
  pros::delay(2000); // Wait for IMU calibration

  // Reset encoders
  forwardTracker->reset_position();
  horizontalTracker->reset_position();  
  pros::delay(500);  // Stop the user from doing anything while legacy ports configure
  mcl->initializeParticles(0,0,0);
  chassis.odom_tracking_set(updateMCL);
  // Are you using tracking wheels?  Comment out which ones you're using here!
